...
// }
// echo "total " . $cart_list . ", ";
// echo "value " . $qty . ", ";
// echo "index " . $id . "<br>";
// print_r($_SESSION['user']);
?>
  </p>
  <div class="album py-5 bg-light shadow-lg p-3 mb-5 rounded">
      <div class="container">
              <?php
                include("admin/conf/db_conf.php");
        ...
        <div class="row  mt-5">
          <div class="col-12">
            <div class="card shadow-sm">
              <!-- ပုံဟာ responsive ဖြစ်တယ် -->
              <a href="shopping.php?id=<?php echo $row['id'] ?>" class="pic_link">
              <img src="admin/cate_pic/<?php echo $row['cate_pic'] ?>" class="bd-placeholder-img card-img-top" width="100%" height="320" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Placeholder: Thumbnail" preserveAspectRatio="xMidYMid slice" focusable="false"><title>Placeholder</title><rect width="100%" height="100%" fill="#55595c"/>
              <text class="ps-4 fs-4 fw-bolder" x="50%" y="50%" fill="#eceeef" dy=".3em"><?php echo $row['category_name'] ?></text></img>
              </a>
              
              <div class="card-body">
                <div class="mt-3 d-flex justify-content-between align-items-center">
                  <div class="btn-group">
                    <a href="shopping.php?id=<?php echo $row['id'] ?>" type="button" class="shopping_btn btn btn-sm btn-primary fw-bolder">Market</a>
                    <!-- <button type="button" class="btn btn-sm btn-outline-secondary">Edit</button> -->
                  </div>
                  <small class="fw-bolder text-secondary" >Qty
                    <span class="ms-1 bg-success ps-2 pe-2 pb-3 pt-3 text-white fw-bolder border border-warning">
                    <?php
                    //to show total of each category item : 
                    // mysqli_num_rows can perform action like loop and then record of rows that looped.
                      $category_id = $row['id'];
                      $sql = mysqli_query($conn,"SELECT * FROM funiture_products WHERE category_id = $category_id");
