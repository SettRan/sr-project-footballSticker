  ...
  <div class="row">
      ...
      <div class="position-sticky pt-3">
        ...
        <!-- covered By logo so cannt see it. -->
          ...
          <li class="nav-item mt-2">
            <a class=" fw-bolder fs-6 " href="category_new.php?number=5">
    ...
    <div class="cate_css container mt-5 mb-5">
      <div class="row">
        ...
        <form action="cate_update.php" enctype="multipart/form-data" method="post">

          <?php
            include('conf/db_conf.php');
            $edit_id = $_GET['edit_id'];
            $category_item = mysqli_query($conn,"SELECT * FROM categories WHERE id=$edit_id");
            $row = mysqli_fetch_assoc($category_item);
          ?>
          <div class="input-group input-group-lg mt-5">
            <span class=" bg-warning fw-bold input-group-text" id="inputGroup-sizing-lg ">ပြင်ဆင်မယ်</span>
            <!-- ပြင်ဆင်မယ့် id ကို hidden လုပ်ပြီးထည့်သွင်း -->
            <input type="hidden" name="cate_id" value="<?php echo $row['id'] ?>" >
            <!-- နာမည်ကို db မှထုတ်ယူ -->
            <input value="<?php echo $row['category_name'] ?>" name="category_name" type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-lg">
          </div>
          <div class="mb-3 mt-3">
                <label for="formFile" class="text-warning form-label ">.....ဓါတ်ပုံရွေးချယ်ရန်.....</label>
                <input type="file" name="picture" class="form-control" id="formFile" >
          </div>
          <img class="rounded-circle" src="cate_pic/<?php echo $row['cate_pic'] ?>" alt="" width="100px">
          <div class="d-grid gap-2 mt-5 mb-5 cate_btn">
            <button class="btn btn-danger fw-bolder" type="submit">ပြန်လည်တည်းဖြတ်ပြီး </button>
            <!-- if button tag, will action to form -->
            <!-- category_new.php ထဲကို number အရေတွက်ပါ သတ်မှတ်ပြီးတော့ id နဲ့ပါချိတ်ထား -->
            <a href="category_new.php?number=2&#cate_inner_link"  href="" class="btn btn-primary fw-bolder" >မလုပ်တော့</a>
          </div>
        </form>
...
<!-- <script>
  function myFunction(){
    alert("အမျိုးအစားအသစ်တစ်ခုအစားထိုးလိုက်ပါလိမ့်မည်...");
  }
</script> -->
