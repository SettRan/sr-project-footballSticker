<?php
  ...
  $cart_list=0; // need it for clear err. if not has it, will be undefined index.
  session_start();
...
<header class="site-header sticky-top py-1">
  <nav class="nav_header container d-flex flex-column flex-md-row justify-content-between" id="up">
    ...
    <a href="view_cat.php" title="Cart" class="ms-5 ps-5 py-2 d-none d-md-inline-block pt-4"><span class="p3 ps-1 pe-1 rounded-pill bg-danger text-light"><?php echo $cart_list ?></span><svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="currentColor" class="bi bi-cart4" viewBox="0 0 16 16">
...
<main>
  ...
  <div class="album py-5 bg-light">
        ...
        // if(isset($_GET['selection_cate'])) {
          ...
          $funiture_table = mysqli_query($conn,"SELECT funiture_products.*, categories.category_name FROM funiture_products LEFT JOIN categories ON funiture_products.category_id = categories.id WHERE funiture_products.category_id = $shop_id ORDER BY funiture_products.modified_date DESC ");

        $cate_result = mysqli_query($conn,"SELECT * FROM categories");
        ?>
      <div class="container">
        <div class="row">
          <?php while($row = mysqli_fetch_assoc($funiture_table)) : ?>
          <div class="col-sm-12 col-md-6 col-lg-4">
            <div class="card shadow-sm shadow-lg p-3 mb-5 bg-white rounded">
              <a title="အသေးစိတ်ကြည့်မယ်" href="detail_shopping.php?funi_id=<?php echo $row['funi_id'] ?>" class="text-decoration-none">
              <img src="admin/funiture_pic/<?php echo $row['picture'] ?>" class="bd-placeholder-img card-img-top" width="100%" height="225" role="img" aria-label="Placeholder: Thumbnail" preserveAspectRatio="xMidYMid slice" focusable="false"><title>Sr Project</title><rect width="100%" height="100%" fill="#55595c"/><text x="50%" y="50%" fill="#eceeef" dy=".3em"><span class="text-dark"><?php echo $row['funi_item'] ?></span><span class="ms-5 fs-6 fw-bold">By</span><div class="mt-2 text-secondary"><?php echo $row['maker'] ?></div></text></img>
              </a>
              ...
              <div class="card-body">
                ...
                <div class="mt-3 d-flex justify-content-between align-items-center">
                  <div class="btn-group">
                    <!-- for addtocard session -->
                    <a href="addtocard.php?id=<?php echo $row['funi_id'] ?>" type="button" class="btn btn-sm btn-warning fw-bolder">Add to Card <svg class="ms-1 fw-bolder" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-clipboard-plus" viewBox="0 0 16 16">
                  ...
                  <small class="text-success fw-bolder"><?php echo $row['price'] ?> <span class="text-dark fw-bolder">(kyats)</span>
                  </small>
                </div>
              </div>
            </div>
